...
const RelaxAnimation: React.FC<RelaxAnimationProps> = ({ show }) => {
  ...
  // Select a random image when the component mounts or show changes
  useEffect(() => {
    if (show) {
      const newRandomImage = Math.floor(Math.random() * 3) + 1;
      setRandomImage(newRandomImage);
      console.log(`Selected random image: ${newRandomImage}`);
    }
  }, [show]);
  
  if (!show) {
    console.log("RelaxAnimation: show is false, not rendering");
    return null;
  }
  
  console.log(`RelaxAnimation: Showing with image ${randomImage}`);
            ...
            <div className="text-center">
              ...
              >
                <img 
                  src={imageUrl} 
                  alt="Relaxing image" 
                  className="w-full h-auto object-cover rounded-lg max-h-64 transform transition-all duration-500 hover:scale-105"
                />
              </motion.div>
